...
%created 11-27-18 %modified 03-02-19
close all
...
if noRuns == 1
    ...
    [~,sweepTau]=min(abs(sweepDelta(:,2)+100)); %finds sweep index closest to -100pA sweep -- for tau-m
...
elseif noRuns == 2
    ...
    [~,sweepTau]=min(abs(sweepDelta.RunOne(:,2)+100)); %finds sweep index closest to -100pA sweep -- for tau-m
...
%tau, taken at sweep closest to -100 pA in run with smaller steps
if noRuns == 1
    output.mtau=getTau(output.ccFile,sweepTau,injstart,samplerate,cRange);
elseif noRuns == 2
    output.mtau=getTau(output.ccFile.RunOne,sweepTau,injstart,samplerate,cRange.RunOne);
end
...
%FR adaptation, ratio of first first ISI to last 2 ISI (<1 slowing down; >1 speeding up)
if noRuns == 1
...
%ratio of last 2 ISI to first ISI in sweep
...
if length(ISI) >= 2
    ratio=ISI(1)/mean([ISI(end) ISI(end-1)]);
else
    ratio=NaN;
end
end